...
jump :: Parser ()
jump = skipMany $ choice [spaces, cmtL lispdef, cmtB lispdef]

-- | Wrapper for lexemes
lexeme :: Parser a -> Parser a
lexeme p = p <* end
 where
  end = gap <|> (void . try $ string ")")

-- | NIL parser
p'nil :: Parser Sexp
p'nil = NIL <$ lexeme (string "nil")

-- | Bool parser
p'bool :: Parser Sexp
p'bool = Bool <$> (lexeme (string "t") $> True)

-- | Integer parser
p'int :: Parser Sexp
p'int = Int <$> lexeme (integer <* option mempty (string "."))

-- | Float parser
p'float :: Parser Sexp
p'float = Float <$> lexeme floatB
...
-- | Char Literal parser
p'char :: Parser Sexp
p'char = Char <$> lexeme (symbol "#\\" *> (p'char'extra <|> count 1 anychar))

-- | String Literal parser
p'str :: Parser Sexp
p'str = String <$> stringLit' lispdef
...
p'keyword = Keyword <$> ((:) <$> char ':' <*> identifier lispdef)
...
p'cons :: Parser Sexp
p'cons =
  between (symbol "(") (symbol ")") (manyTill' dot p'sexp)
    >>= \(xs, a) -> pure $ foldr Cons a xs
 where
  dot =
    p'sexp >>= \a -> symbol "." *> p'sexp >>= \b -> pure $ Cons a b
...
f'set :: Fn
f'set s =
  g'binary s >>= bimapM eval >>= \t@(_, x) -> case x of
...
f'makunbound :: Fn
f'makunbound s =
  g'unary s >>= eval >>= g'symbol >>= \t@(_, a@(Symbol k)) ->
    del'genv k t >>= put a
...
f'boundp :: Fn
f'boundp s =
  g'unary s >>= eval >>= g'symbol >>= \t@(_, Symbol k) ->
...
f'append :: Fn
f'append s =
  g'nary s >>= mapM' eval >>= \t@(_, x) -> case x of
    [] -> put NIL t
    [a] -> put a t
    a ->
      put (init a) t >>= modify (pure . List) >>= concat' >>= get >>= \case
...
f'mapcar :: Fn
f'mapcar s =
  g'nary s >>= mapM' eval >>= \t@(_, f : xs) -> case xs of
    a
      | not $ all listp' a ->
...
f'if s =
  g'nary s >>= get >>= \case
    ...
    a : if' : else' ->
      put a s >>= eval >>= t'nil >>= \t@(_, v) -> case v of
...
f'unless s =
  g'nary s >>= get >>= \case
    ...
    [a] -> put a s >>= eval >>= put NIL
    a : rest ->
      put a s >>= eval >>= nil't >>= \t@(_, v) -> case v of
        NIL -> put NIL t
        _ -> put rest t >>= eval'body
...
replace'cond d n f s =
  get s >>= \x -> case compare' x d of
    LT -> pure s
    EQ -> replace d n f s
...
replace d n f s =
  get s >>= \case
    ...
    At a ->
      put a s >>= eval >>= \t@(_, x) -> case x of
        List ls -> put ls t >>= mapM' (modify (pure . f)) >>= modify (pure . List)
        a
          | n > 1 -> err [errEval, errNotList, show' a]
          | otherwise -> pure t
    _ -> pure s
...
splice d = go []
 where
  go r s@(_, xs) = case xs of
    [] -> put (concat . reverse $ r) s >>= modify (pure . List)
    x : rest ->
      put x s >>= replace'cond d (length xs) id >>= \t@(_, v) ->
        case v of
          a@(List l)
            | spliceable x -> put rest t >>= go (l : r)
            | otherwise -> put rest t >>= go ([a] : r)
          a
            | length xs == 1 && spliceable x -> put a t
            | otherwise -> put rest t >>= go ([a] : r)
...
sl = do
  ...
  runInputT
    (defaultSettings {historyFile = Just historyFile})
